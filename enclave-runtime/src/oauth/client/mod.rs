...
fn start_client() {
    // println!("HTTP CLIENT: {:?}", http_client);
    println!("The client is starting");
    let github_client_id = ClientId::new(
        config::GITHUB_CLIENT_ID.to_string(),
    );
    let github_client_secret = ClientSecret::new(
        config::GITHUB_CLIENT_SECRET.to_string(),
    );
    println!("Github client ID: {:?}", github_client_id);
